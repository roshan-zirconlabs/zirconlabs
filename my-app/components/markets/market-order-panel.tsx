...
const field = "c-input mt-2";
const button = "c-btn-ghost";
...
export default function MarketOrderPanel() {
  ...
  return <section className="space-y-8" aria-labelledby="market-order-title">
    <h2 id="market-order-title" className="c-serif text-3xl text-white">1 · Choose a market</h2>
    <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={e => { e.preventDefault(); void search(); }}>
      <label className="flex-1 text-sm text-[var(--c-dim)]">Search markets<input name="marketSearch" type="search" maxLength={120} autoComplete="off" value={query} onChange={e => setQuery(e.target.value)} placeholder="Try Bitcoin, elections or sports…" className={field} /></label>
      <button className={button} disabled={!!busy || !!requestId}>{busy === "search" ? "Searching…" : "Search"}</button>
    </form>
    {markets.length > 0 ? <label className="block text-sm text-[var(--c-dim)]">Market<select name="market" className={field} disabled={!!busy || !!requestId} value={slug} onChange={e => { setSlug(e.target.value); setOutcome(""); invalidate(); }}><option value="">Choose a market</option>{markets.map(m => <option key={m.slug} value={m.slug}>{m.question}</option>)}</select></label> : !busy && <p role="status" className="text-sm text-[var(--c-dim)]">No available markets found. Try another search.</p>}
    {selected && <form className="space-y-5" onSubmit={e => { e.preventDefault(); void preview(); }}>
      <fieldset disabled={!!busy || !!requestId}><legend className="c-serif text-3xl text-white">2 · Your prediction</legend><div className="mt-2 flex flex-wrap gap-3">{selected.outcomes.map(o => <label key={o.label} className="flex min-h-11 cursor-pointer items-center gap-2 rounded-full border border-white/15 px-5 py-2.5 text-sm transition-colors hover:border-white/30 has-[:checked]:border-[rgba(247,168,207,0.6)] has-[:checked]:bg-[rgba(224,97,159,0.12)]"><input type="radio" name="outcome" required value={o.label} checked={outcome === o.label} onChange={() => { setOutcome(o.label); invalidate(); }} />{o.label}</label>)}</div></fieldset>
      <fieldset disabled={!!busy || !!requestId} className="grid gap-4 sm:grid-cols-2"><legend className="sr-only">Order amount and price protection</legend><label className="text-sm text-[var(--c-dim)]">Amount (USD)<input className={field} name="amount" type="number" inputMode="decimal" required min="1" max="100" step="0.01" value={amount} onChange={e => { setAmount(e.target.value); invalidate(); }} /></label><label className="text-sm text-[var(--c-dim)]">Maximum price per share (cents)<input className={field} name="priceLimit" type="number" inputMode="decimal" required min="0.01" max="99.99" step="0.01" value={ceiling} onChange={e => { setCeiling(e.target.value); invalidate(); }} /></label></fieldset>
      <button className="c-btn-primary" disabled={!!busy || !!requestId}>{busy === "preview" ? "Checking prices…" : "Preview order"}</button>
    </form>}
    {quote && <div className="space-y-4 rounded-3xl border border-[rgba(247,168,207,0.3)] bg-black/25 p-6"><h3 className="c-serif text-3xl text-white">3 · Review your order</h3><p className="text-sm text-[var(--c-dim)]">{quote.question} <strong>{quote.outcome}</strong></p><dl className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4"><div><dt className="text-[var(--c-faint)]">Order amount</dt><dd className="c-mono mt-1 text-lg text-white">${quote.amountUsd.toFixed(2)}</dd></div><div><dt className="text-[var(--c-faint)]">Estimated shares</dt><dd className="c-mono mt-1 text-lg text-white">{quote.estimatedShares.toFixed(2)}</dd></div><div><dt className="text-[var(--c-faint)]">Average price</dt><dd className="c-mono mt-1 text-lg text-white">{(quote.estimatedAveragePrice * 100).toFixed(2)}¢</dd></div><div><dt className="text-[var(--c-faint)]">Price ceiling</dt><dd className="c-mono mt-1 text-lg text-white">{(quote.maxPrice * 100).toFixed(2)}¢</dd></div></dl><p className="text-xs leading-5 text-[var(--c-faint)]">{quote.note} Market minimum: {quote.minimumShares} shares. The order buys the full requested amount or is cancelled if it cannot be filled.</p>
      {expired && <p role="status" className="text-sm text-amber-700">This preview has expired. Preview again to refresh prices.</p>}
      {status !== "authenticated" ? <Link className="c-btn-primary" href="/auth/sign-in?callbackUrl=/trade">Sign in to trade</Link> : <><p className="text-sm text-[var(--c-dim)]">Your <Link href="/wallet" className="text-[var(--c-pink)] underline">trading account</Link> must be funded, approved and enabled for live trading.</p><label className="flex items-start gap-3 text-sm text-white"><input type="checkbox" className="mt-1 h-4 w-4" checked={confirmed} disabled={!!requestId} onChange={e => setConfirmed(e.target.checked)} /><span>I confirm this {quote.outcome} purchase using real funds. Fees may apply.</span></label><button onClick={() => void submit()} disabled={!!busy || !confirmed || expired || !!requestId} className="c-btn-primary">{busy === "buy" ? "Submitting order…" : `Buy ${quote.outcome} for $${quote.amountUsd.toFixed(2)}`}</button></>}
    </div>}
    {requestId && <div className="space-y-3 rounded-3xl border border-white/15 bg-black/25 p-6"><p className="c-eyebrow">Order request</p><p className="c-mono break-all text-xs text-[var(--c-dim)]">{requestId}</p><button type="button" onClick={() => void checkOrder()} disabled={!!busy} className={button}>Check order status</button>{receipt && <div role="status" className="space-y-2 text-sm text-white"><p>{receipt.ok ? "Order accepted. Check settlement receipts below." : receipt.message || receipt.error || receipt.status}</p>{receipt.orderId && <p className="break-all text-xs">Order: {receipt.orderId}</p>}{receipt.transactionHashes?.filter(hash => /^0x[0-9a-f]{64}$/i.test(hash)).map(hash => <a key={hash} href={`https://polygonscan.com/tx/${hash}`} target="_blank" rel="noreferrer" className="block text-[var(--c-pink)] underline">View settlement transaction</a>)}{receipt.ok && !receipt.transactionHashes?.length && <p>Settlement is not yet verified. Acceptance alone does not confirm a fill.</p>}{(receipt.ok || ["REJECTED", "NOT_SUBMITTED"].includes(receipt.status)) && <button className={button} onClick={newOrder}>Review a new order</button>}</div>}</div>}
    {error && <p role="alert" className="rounded-2xl border border-rose-300/50 bg-rose-50 p-4 text-sm text-rose-800">{error}</p>}
  </section>;
}
