...
export default function TrackRecordView({ record, showProof }: { record: TrackRecord; showProof?: boolean }) {
  ...
  const successRate = record.runs > 0 ? Math.round((record.okRuns / record.runs) * 100) : null;

  return (
    <div>
      <p className="mb-4 text-sm text-[var(--c-dim)]">
        <span className="text-white">{record.runs}</span> runs
        {successRate != null && (
          <>
            {" · "}
            <span className="text-white">{successRate}%</span> succeeded
          </>
        )}
        {" · "}last active <span className="text-white">{since(record.lastRunAt)}</span>
      </p>

      {record.trades === 0 ? (
        <p className="c-panel p-5 text-sm text-[var(--c-faint)]">
          This workflow isn&rsquo;t a Polymarket trading strategy — showing run activity only.
        </p>
      ) : (
        <>
