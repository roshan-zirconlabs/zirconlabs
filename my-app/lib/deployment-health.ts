...
export function checkDeploymentConfig(env: Environment, requestUrl: string) {
  const required = { DATABASE_URL: env.DATABASE_URL, AUTH_SECRET: env.AUTH_SECRET || env.NEXTAUTH_SECRET, GOOGLE_CLIENT_ID: env.GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET: env.GOOGLE_CLIENT_SECRET, ENCRYPTION_KEY: /^[a-f0-9]{64}$/i.test(env.ENCRYPTION_KEY ?? "") ? "valid" : "" };
  ...
  return { missing, issues, callbackUrl };
}
