...
export default async function StrategyPage({ params }: { params: Promise<{ workflowId: string }> }) {
  ...
  if (key) {
    try {
      ...
      record = await computeTrackRecord(raw);
    } catch {
      /* record stays null */
    }
  }
    ...
    <PageShell>
      ...
      <section className="c-panel mt-6 p-6">
        ...
        <p className="mt-1 text-sm text-[var(--c-dim)]">
          Query this record before you subscribe — it returns per-trade proof (KeeperHub execution ids and on-chain
          condition ids) so you can verify it yourself, no trust required.
        </p>
